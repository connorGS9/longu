...
# Static Analysis: Individualism of a language starts to appear.. Static analysis takes the finished, well-formed AST and studies it for meaning, catching errors the parser couldn't without running any of the code (static) and makes sure that the AST actually makes sense
 $ if our AST produced: (×)  according to the parser this is valid we are just multiplying two variables, but the SA will actually dive into those variables and determine if they even exist, if they are 64 Bit ints, or 32 Bit ints (IN STATICALLY TYPED LANGUAGES), according to where they are stored.
                       /   \
                  shares   close
